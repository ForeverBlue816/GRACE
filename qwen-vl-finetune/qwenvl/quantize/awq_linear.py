...
Why vendored: `import awq` eagerly runs `from awq.models import *`, which pulls a
large set of model classes and heavy optional dependencies; we only need the
`WQLinear_GEMM` kernel module for inference, so we copy it here verbatim. The
packing layout is byte-for-byte identical to AutoAWQ, so the optional `awq_ext`
CUDA kernels from the `autoawq-kernels` package work unchanged.
