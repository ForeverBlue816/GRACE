"""Convert this repo's group-wise symmetric INT4 LSQ-QAT checkpoint into real
AutoAWQ-packed 4-bit weights (AWQ GEMM layout).

QAT (see llava/train/qat_modules.py) is *symmetric signed*:
...
`qat_quantized_weights.bin` (the training sidecar) stores, per quantized linear:
    <name>.weight     fake-quantized weight W_dq (on the int grid)
    <name>.log_scale  per-group log-scale (length out_features * in_groups)
    <name>.bias       (optional)
We use it as the single source of truth for which layers were quantized.
"""
...
def set_module(model: nn.Module, name: str, new_module: nn.Module) -> None:
    """Replace the submodule at dotted `name` (e.g. model.layers.0.mlp.up_proj)."""
...
def _scales_zeros_from_qat(log_scale: torch.Tensor, out_features: int, in_features: int,
    ...
    QAT group index for weight element (o, i) is  o * n_in_groups + (i // group_size),
    so log_scale.reshape(out_features, n_in_groups)[o, j] is the scale of output o,
...
@torch.no_grad()
def convert_llava_to_awq(model: nn.Module, qat_bin_path: str,
    ...
    # from_pretrained try to import the (broken-under-tf4.37) autoawq package.
    # We load the AWQ weights ourselves, so we must not trigger that path.
...
def maybe_convert_to_awq(model: nn.Module, model_path: str, enable: bool,
    ...
    If `enable` is False, leaves the (fp16 fake-quant baseline) model untouched
    and returns False. If True, packs the LLM linears into real AWQ 4-bit using
    the training sidecar (defaults to `<model_path>/qat_quantized_weights.bin`)
    and returns True. Call it right after load_pretrained_model().
    """
    if not enable:
        return False
    if qat_bin is None:
    ...
    convert_llava_to_awq(model, qat_bin, bits=bits, group_size=group_size, verify=verify)
    model.eval()
    return True
